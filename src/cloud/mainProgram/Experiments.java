...
public class Experiments {
	...
	public static void main(String[] args)throws Exception {
		...
		System.out.println("The experiment starts at: " + cTime.format(new Date()));
		...
		for(alft=0.1; alft<=0.51; alft=alft+0.05) {
			...
			//Repeat 10 times for each group parameters
			for(int r=0;r<Parameters.totalRepeatNum;r++) { 
				//Generate the workflow set and result txt
				...
				for(int i=0;i<Parameters.SCHEDULES.length;i++) {
					...
					ByteArrayInputStream fi = new ByteArrayInputStream(Parameters.experimentWorkflowStream.toByteArray()); //Get experiment workflow from stream
		...
		for(vita=0.1; vita<=0.51; vita=vita+0.05) { 
		...
		} //End test the impact of variance of data transfer time
		...
		for(gama=1; gama<=8.01; gama=gama+1) { 
		...
		int[] wfNumGroup = {30,50,100,200,500,1000}; //The workflow count: 30,50,100,200,500,1000
		...
		System.out.println("The experiment finishs at: " + cTime.format(new Date()));
    } //End main
